-- Crew Intake · 0008 · submit_report idempotency key
--
-- The app sends every report through a persisted outbox that retries. A retry of a send
-- that already committed must be a no-op, not a second edit: the outbox passes
-- p_client_ref (a uuid per VERSION of the answers), the function records it in the audit
-- row it writes, and a call carrying a ref it has already seen returns the existing
-- submission untouched (duplicate = true). New answers always come with a new ref.
-- The insert path now also writes an audit row (action 'submit').

drop function if exists public.submit_report(uuid, date, jsonb, jsonb);
  ...
  if found then
    ...
    if p_client_ref is not null and exists (
      ...
      return jsonb_build_object(
        ...
        'edited', false, 'duplicate', true, 'editable_until', v_deadline
      );
    end if;
...
revoke execute on function public.submit_report(uuid, date, jsonb, jsonb, uuid) from public, anon;
grant  execute on function public.submit_report(uuid, date, jsonb, jsonb, uuid) to authenticated, service_role;
